...
function App() {
  ...
  const nav = [
    ["inicio", "P01 - Inicio"], ["dashboard", "P15 - Dashboard KPI"],
    ...(showOperations ? [["operacion", "P02-P12 - Operacion e historial"]] : []),
    ...(showLaboratory ? [["laboratorio", "P21/P22 - Laboratorio P21/P22"]] : []),
    ...(showTraceability ? [["trazabilidad", "P16 - Trazabilidad"]] : []),
    ...(showMaintenance ? [["mantenimiento", "P08 - Mantenimiento M7"]] : []),
    ...(canSupervise && !session.remote ? [["supervision", "P13/P14/P24 - Supervision"]] : []),
    ...(canAdmin && !session.remote ? [["administracion", "P17-P25 - Administracion"]] : []),
  ];
...
function Supervision({ request, admin, setMessage }: { request: <T>(r: string, p?: unknown, m?: "GET" | "POST" | "PUT" | "PATCH") => Promise<T>; admin: boolean; setMessage: (value: string) => void }) {
  ...
  async function load() { try { const response = await request<unknown>(routes[tab]); setRows(Array.isArray(response) ? response : [response]); } catch (error) { setMessage(errorText(error)); } }
...
function Administration({ request, setMessage }: { request: <T>(r: string, p?: unknown, m?: "GET" | "POST" | "PUT" | "PATCH") => Promise<T>; setMessage: (value: string) => void }) {
  const [tab, setTab] = useState("P19 · usuarios"); const [rows, setRows] = useState<unknown[]>([]); const [catalogType, setCatalogType] = useState("formato"); const [code, setCode] = useState(""); const [description, setDescription] = useState("");
  const routes: Record<string, string> = { "P17 · catalogos": "/catalogos/formato", "P18 · limites": "/limites", "P19 · usuarios": "/usuarios", "P19 · personas": "/personas", "P19 · roles": "/roles", "P20 · auditoria": "/auditoria", "P21/P22 · laboratorio": "/laboratorio/puntos-muestreo", "P23 · calendario": "/calendario?desde=2026-09-01&hasta=2026-09-30", "P25 · parametros": "/configuracion/baseline", "P25 · PLC": "/plc/configuracion" };
  ...
  return <section className="content"><h2>P17-P25 · Administracion</h2><p>Catalogos, limites, identidad, auditoria, laboratorio, calendario, parametros y PLC. El baseline v1.0.1 conserva procedencia y pendientes sin sobrescribir configuracion existente.</p><div className="tabbar">{Object.keys(routes).map((item) => <button type="button" key={item} className={tab === item ? "active" : ""} onClick={() => { setTab(item); setRows([]); }}>{item}</button>)}</div><div className="split"><section><button type="button" onClick={load}>Consultar {tab}</button><div className="result-list">{rows.map((row, index) => <article key={index}><pre>{JSON.stringify(row, null, 2)}</pre></article>)}</div></section><form onSubmit={createCatalog}><h3>P17 · Alta de catalogo</h3><label>Tipo<input value={catalogType} onChange={(event) => setCatalogType(event.target.value)} required /></label><label>Codigo<input value={code} onChange={(event) => setCode(event.target.value)} required /></label><label>Descripcion<input value={description} onChange={(event) => setDescription(event.target.value)} required /></label><button>Crear catalogo</button></form></div></section>;
}
