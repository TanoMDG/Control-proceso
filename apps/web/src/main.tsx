...
function Operations({ request, session, setMessage }: { request: <T>(r: string, p?: unknown, m?: "GET" | "POST" | "PUT" | "PATCH") => Promise<T>; session: Session; setMessage: (value: string) => void }) {
  const modules = operationsFor(session); const [module, setModule] = useState(modules[0] ?? "M1"); const [values, setValues] = useState<Record<string, string>>({}); const [rows, setRows] = useState<RecordRow[]>([]); const [shift, setShift] = useState("ACTUAL"); const [editing, setEditing] = useState<RecordRow | null>(null);
  ...
  function datosFromValues() { const datos: Record<string, unknown> = {}; for (const [key, , type] of fieldsFor(module)) { const value = values[key]?.trim(); if (!value) continue; datos[key] = type === "number" ? Number(value) : type === "datetime-local" ? new Date(value).toISOString() : key === "detalles" ? JSON.parse(value) : value; } return datos; }
  async function submit(event: FormEvent) { event.preventDefault(); try { const datos = datosFromValues(); if (editing) { await request(`/registros/${module.toLowerCase()}/${editing.id}`, { revision: editing.revision, datos }, "PUT"); setMessage(`${module} corregido.`); } else { const payload = { client_uuid: clientUuid(), fecha_operativa: today(), turno_codigo: shift, instante_medicion: now(), datos, origen_dato: "digital_directo" }; await request(`/registros/${module.toLowerCase()}`, payload, "POST"); setMessage(`${module} registrado como borrador.`); } setEditing(null); setValues({}); await load(); } catch (error) { if (editing) { setMessage(`No se pudo corregir ${module}: ${errorText(error)}`); return; } const payload = { client_uuid: clientUuid(), fecha_operativa: today(), turno_codigo: shift, instante_medicion: now(), datos: values, origen_dato: "digital_directo" }; if (!navigator.onLine) { await savePending({ ...makePending(payload.client_uuid, module.toLowerCase(), payload, errorText(error)), route: `/registros/${module.toLowerCase()}`, method: "POST" }); setMessage(`${module} quedo en cola local.`); } else setMessage(`No se pudo registrar ${module}: ${errorText(error)}`); } }
  ...
  return <section className="content"><h2>Operacion e historial</h2><div className="split"><form onSubmit={submit}><label>Modulo<select value={module} onChange={(event) => { setModule(event.target.value); setValues({}); setEditing(null); }}>{modules.map((item) => <option key={item}>{item}</option>)}</select></label><label>Turno<input value={shift} onChange={(event) => setShift(event.target.value)} required /></label>{fieldsFor(module).map(([key, label, type]) => <label key={key}>{label}{type === "textarea" ? <textarea value={values[key] ?? ""} onChange={(event) => setValues({ ...values, [key]: event.target.value })} rows={4} /> : <input type={type} step={type === "number" ? "any" : undefined} value={values[key] ?? ""} onChange={(event) => setValues({ ...values, [key]: event.target.value })} required={!label.includes("opcional") && !["humedad", "temperatura", "humedad_ksider", "segundos_pesada", "silo", "altura_libre_m", "linea", "fin"].includes(key)} />}</label>)}<button>{editing ? `Guardar correccion ${module}` : `Guardar ${module}`}</button>{editing && <button type="button" className="secondary" onClick={() => { setEditing(null); setValues({}); }}>Cancelar correccion</button>}</form><section><div className="form-actions"><button type="button" onClick={load}>Actualizar historial</button><a href={`${apiUrl}/exportar?modulo=${module.toLowerCase()}`} target="_blank" rel="noreferrer">Imprimir formulario</a></div><div className="result-list">{rows.map((row) => <article key={row.id}><strong>{row.fecha_operativa} · {row.turno_codigo} · {row.estado}</strong><span>Revision {row.revision}</span><pre>{JSON.stringify(row.datos, null, 2)}</pre><div className="form-actions">{row.estado === "BORRADOR" && <button type="button" className="secondary" onClick={() => action(row, "cerrar")}>Cerrar</button>}{row.estado === "BORRADOR" && <button type="button" className="secondary" onClick={() => edit(row)}>Editar</button>}{session.role !== "CARGA" && row.estado === "CERRADO" && <button type="button" className="secondary" onClick={() => action(row, "validar")}>Validar</button>}{session.role !== "CARGA" && row.estado !== "ANULADO" && <button type="button" className="danger" onClick={() => action(row, "anular")}>Anular</button>}</div></article>)}</div></section></div></section>;
}
...
function Traceability({ request, session, setMessage }: { request: <T>(r: string, p?: unknown, m?: "GET" | "POST" | "PUT" | "PATCH") => Promise<T>; session: Session; setMessage: (value: string) => void }) {
  const [mua, setMua] = useState<Array<{ id: string; codigo: string; presencias_activas: Array<{ box: string; desde: string }>; orden_fifo: number | null }>>([]); const [muaId, setMuaId] = useState(""); const [box, setBox] = useState(""); const [preparers, setPreparers] = useState<Array<{ id: string; apellido_nombre: string }>>([]); const [preparerId, setPreparerId] = useState(""); const [component, setComponent] = useState("");
  async function load() { try { const rows = await request<typeof mua>("/mua"); setMua(rows); if (session.role !== "CARGA") { const people = await request<typeof preparers>("/trazabilidad/preparadores"); setPreparers(people); setPreparerId((current) => current || people[0]?.id || ""); } } catch (error) { setMessage(`No fue posible cargar MUA: ${errorText(error)}`); } }
  ...
  return <section className="content"><h2>Trazabilidad MUA</h2><p>La secuencia conserva periodos y certeza; no calcula proporciones ni toneladas.</p><div className="split"><section><button type="button" onClick={load}>Actualizar MUA y FIFO</button><div className="result-list">{mua.map((item) => <article key={item.id}><strong>{item.codigo}</strong><span>FIFO: {item.orden_fifo ?? "sin box"}</span>{item.presencias_activas.map((presence) => <span key={presence.box}>Box {presence.box} desde {presence.desde}</span>)}</article>)}</div></section><section>{session.role !== "CARGA" && <form onSubmit={create}><h3>M4 · Crear MUA</h3><label>Preparador M4<select value={preparerId} onChange={(event) => setPreparerId(event.target.value)} required><option value="">Seleccione preparador</option>{preparers.map((person) => <option key={person.id} value={person.id}>{person.apellido_nombre}</option>)}</select></label><label>Componente<input value={component} onChange={(event) => setComponent(event.target.value)} required /></label><button disabled={!preparers.length}>Crear MUA</button></form>}<form onSubmit={place}><h3>M5 · Cargar MUA en box</h3><label>MUA<select value={muaId} onChange={(event) => setMuaId(event.target.value)} required><option value="">Seleccione una MUA</option>{mua.map((item) => <option key={item.id} value={item.id}>{item.codigo}</option>)}</select></label><label>Box<input value={box} onChange={(event) => setBox(event.target.value)} required /></label><button>Registrar presencia</button></form></section></div></section>;
}
